...
<br><h1 style="text-align:center">Article(s) contenant "<?= $_GET['search'] ?>".</h1><br><br>
    <!-- New Arrivals Area -->
		<!-- ZONE RECHERCHE  -->
        <div class="container" id="recherche">
            ...
            <?php
                ...
                $stmt = $pdo->prepare('SELECT * from produits WHERE nom LIKE "%'.$_GET['search'].'%" OR categorie LIKE "%'.$_GET['search'].'%" ORDER BY recommande DESC');
                $stmt->execute();
            ...
            ?>
            <h2>Nombre d'articles trouvés : <?= count($resultatsRecherche) ?></h2>
            <div class="row">
                <div class="col-12">
                    <div class="new_arrivals_slides owl-carousel">
                        <?php
                        foreach($resultatsRecherche as $res){
						?>
